...
public function ちょうど255文字は投稿できる()
{
    $seller = User::factory()->create();
    $user   = User::factory()->create([
        'email_verified_at' => now(),
    ...
    $this->actingAs($user)
        ->post(route('products.comments.store', $product), ['body' => $ok])
...
public function ログイン済みでもメール未確認なら投稿できず認証画面へリダイレクト()
{
    $seller = User::factory()->create();
    $user   = User::factory()->create([
    ...
    $this->actingAs($user)
        ->post(route('products.comments.store', $product), ['body' => 'テスト'])
